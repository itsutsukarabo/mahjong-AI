"""
手牌類推モデル v23: v22 特徴量 (442次元) + ターツブロック (134ブロック)

v22からの変更点:
  - N_BLOCKS: 89 → 134 (ターツ45種追加: リャンメン24 + カンチャン21)
  - データ: hand_inference_v23.ndjson (--include-tatsu で生成)
  - build_block_selection_matrix(include_tatsu=True) で 134×34 行列を使用

v22 の設定を引き継ぐ:
  - input_dim=442, LAMBDA_BLOCK=0.3, pos_weight上限10, block_head 2層
"""
...
from enumerate_decompositions import build_block_selection_matrix, N_BLOCKS_WITH_TATSU as N_BLOCKS

# ---- 設定 ----

DATA_DIR  = Path(__file__).parent.parent / "data" / "features"
MODEL_DIR = Path(__file__).parent.parent / "models" / "hand_inference" / "v23"
...
CONFIG = {
    ...
    "n_count_cls":  5,
    "n_players":    3,
    "n_blocks":     N_BLOCKS,  # N_BLOCKS_WITH_TATSU = 134
    "dropout":      0.1,
    "lr":           1e-3,
    "weight_decay": 1e-4,
    "batch_size":   256,
...
LAMBDA_RED_CE      = 0.3
LAMBDA_RED_CONS    = 0.1
LAMBDA_BLOCK       = 0.3   # v21 から引き継ぐ
POS_WEIGHT_MAX     = 10.0  # v21 から引き継ぐ
GRAD_CLIP_NORM     = 1.0
...
class HandInferenceV23(nn.Module):
    def __init__(self, input_dim, d_model, nhead, num_layers,
        ...
        block_sel = torch.tensor(build_block_selection_matrix(include_tatsu=True), dtype=torch.float32)
...
def main():
    ...
    ndjson_path = DATA_DIR / "hand_inference_v23.ndjson"
    if not ndjson_path.exists():
        print(f"ファイルなし: {ndjson_path}")
        print("python phase2/scripts/add_block_labels.py --src ... --dst hand_inference_v23.ndjson --include-tatsu を先に実行してください")
        sys.exit(1)
