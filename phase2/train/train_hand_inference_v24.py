"""
手牌類推モデル v24: v15 ロジック (ブロックヘッドなし) + 442次元特徴量

v15からの変更点:
  - input_dim: 374 → 442 (pass_chi_signal 34次元 + chi_called_tile_signal 34次元追加)
  - データ: hand_inference_v22.ndjson (v22 データパイプライン)

v15 の設定を引き継ぐ:
  - ブロック予測ヘッドなし (LAMBDA_BLOCK なし)
  - eval_by_remaining バケット別分析あり
  - 損失: NLL + sum(λ=0.05) + cross(λ=0.1) + red_ce(λ=0.3) + red_cons(λ=0.1)
"""
...
MODEL_DIR = Path(__file__).parent.parent / "models" / "hand_inference" / "v24"
...
class HandInferenceV24(nn.Module):
...
def eval_by_remaining(model, loader, device):
    ...
    buckets = {k: {"correct": 0, "total": 0, "eae_sum": 0.0, "n_samples": 0} for k in REMAINING_BUCKETS}
...
def main():
    ...
    if resume and (MODEL_DIR / "model.pt").exists() and (MODEL_DIR / "train_log.json").exists():
        ...
        print(f"resume: epoch {start_epoch} から再開  best_val_eae={best_val_eae:.4f}  patience={patience_cnt}", flush=True)
