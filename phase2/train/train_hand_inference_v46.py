"""
手牌類推モデル v46: push/retreat(BCE×2) → aggression スカラー回帰ヘッド(SmoothL1)

v45 からの変更:
  - push/retreat 2ヘッド(λ=0.1×2) を aggression スカラー回帰ヘッド1つ(λ=2.0)に置換
  - aggression_encoder 入力: 171次元
      score(11) + game_state(9) + self_meld(38) + visible(34) + red_vis(3)
      + other1_meld(38) + other2_meld(38)
    ※ other1/other2 meld を追加して対戦相手副露情報を入力に含める
  - aggression_head: Linear(32,1) + Tanh → 出力 (-1, +1)
  - 損失: SmoothL1, pressure_mask (label_aggression != 0.0) で中立0を除外
  - forward() 戻り値 9→8 値: retreat_logit,push_logit → aggression_logit に集約
  データ: hand_inference_v45.ndjson → hand_inference_v46.ndjson
  (hand_inference_v46.ndjson には label_aggression フィールドが必要)
"""
...
CONFIG = {
    ...
    "token_dim":    HI_TOKEN_DIM,  # 45 (danger_level追加)
...
# aggression_feat: 攻撃性ヘッド入力 (171次元)
# score(11) + game_state(9) + self_meld(38) + visible(34) + red_vis(3)
# + other1_meld(38) + other2_meld(38) = 171
...
SELF_OTHER1_MELD    = HI_OTHER1_MELD   # 278
SELF_OTHER2_MELD    = HI_OTHER2_MELD   # 316
AGGRESSION_FEAT_DIM = HI_SCORE_DIM + HI_GAME_DIM + HI_MELD_DIM + 34 + 3 + HI_MELD_DIM + HI_MELD_DIM  # 171
...
LAMBDA_SHANTEN    = 0.2
LAMBDA_AGGRESSION = 2.0    # SmoothL1スカラー回帰、CE系損失とスケール釣り合わせ
LAMBDA_FURITEN    = 0.4
LAMBDA_YAKU       = 0.1
...
class HandInferenceDataset(Dataset):
    def __init__(self, features, disc_tokens_list,
                 ...
                 noise_mask, aggression_label,
                 labels_yaku, labels_won):
        self.features          = torch.as_tensor(features,          dtype=torch.float32)
        self.disc_tokens       = disc_tokens_list   # List[List[np.ndarray(n,45)]]
        ...
        self.aggression_label  = torch.as_tensor(aggression_label,  dtype=torch.float32)
    ...
    def __getitem__(self, idx):
        return (self.features[idx],
                ...
                self.noise_mask[idx], self.aggression_label[idx],
...
def collate_fn(batch):
    ...
    return (torch.stack(features), padded, mask,
            ...
            torch.stack(noise_mask), torch.stack(aggression_label),
...
class DiscardEncoder(nn.Module):
    ...
    def __init__(self, token_dim=45, disc_d=64, nhead=4, num_layers=2, dropout=0.1):
        super().__init__()
...
class HandInferenceV37(nn.Module):
    ...
    入力:
      x:            (B, 3, fixed_dim=674)
      disc_tokens:  (B, 3, N, token_dim=45)  パディング済み
    ...
    def __init__(self, fixed_dim, token_dim, disc_d, disc_nhead, disc_layers,
        ...
        # aggression: 局面文脈+相手副露 (171次元) からスカラー回帰 (-1〜+1)
        self.aggression_encoder = nn.Sequential(
            nn.Linear(AGGRESSION_FEAT_DIM, 64), nn.ReLU(), nn.Linear(64, 32)
        )
        self.aggression_head = nn.Sequential(nn.Linear(32, 1), nn.Tanh())
    ...
    def forward(self, x, disc_tokens, disc_mask):
        ...
        # 攻撃性ヘッド入力: 局面文脈(score+game+self_meld+visible+red_vis) + 相手副露 (171次元)
        aggression_feat = torch.cat([
            x[:, 0, SELF_SCORE_START  : SELF_SCORE_START  + HI_SCORE_DIM],   # [39:50]  11
            x[:, 0, SELF_GAME_START   : SELF_GAME_START   + HI_GAME_DIM],    # [50:59]   9
            x[:, 0, SELF_SELF_MELD    : SELF_SELF_MELD    + HI_MELD_DIM],    # [59:97]  38
            x[:, 0, SELF_VISIBLE      : SELF_VISIBLE      + 34],             # [97:131] 34
            x[:, 0, SELF_RED_VIS      : SELF_RED_VIS      + 3],              # [134:137] 3
            x[:, 0, SELF_OTHER1_MELD  : SELF_OTHER1_MELD  + HI_MELD_DIM],   # [278:316] 38
            x[:, 0, SELF_OTHER2_MELD  : SELF_OTHER2_MELD  + HI_MELD_DIM],   # [316:354] 38
        ], dim=-1)                                                             # (B, 171)
        agg_h           = self.aggression_encoder(aggression_feat)
        aggression_logit = self.aggression_head(agg_h).squeeze(-1)           # (B,) in (-1,+1)

        return logits, logits_raw, red_logits, block_logits, wait_logits, aggression_logit, shanten_logits, yaku_logits
...
def train_epoch(model, loader, optimizer, device,
    ...
    for batch in loader:
        ...
        logits, logits_raw, red_logits, block_logits, wait_logits, \
            aggression_logit, shanten_logits, yaku_logits = model(features, disc_tok, disc_mask)
        ...
        infer_w_p0 = infer_w[:, 0]   # player 0 の推論重み (aggression/yaku は p0 のみ)

        # aggression SmoothL1: noise除外+推論重み+圧力マスクを既存sample_weight機構に乗せる
        # combined_w = sample_w(ノイズ除外) × infer_w_p0(非公開牌逆数) × pressure_mask(中立0除外)
        pressure_mask   = (aggression_label != 0.0).float()
        combined_w      = sample_w * infer_w_p0 * pressure_mask
        denom_agg       = combined_w.sum()
        loss_aggression = (
            F.smooth_l1_loss(aggression_logit, aggression_label, reduction='none') * combined_w
        ).sum() / denom_agg if denom_agg > 0 else torch.tensor(0., device=device)
        ...
        loss = (loss_nll
                ...
                + LAMBDA_AGGRESSION * loss_aggression
...
def eval_shanten_acc(model, loader, device):
    ...
    for batch in loader:
        ...
        shanten_logits = model(features, disc_tok, disc_mask)[6]   # (B, P, N_SHANTEN_CLASSES)
...
@torch.no_grad()
def eval_aggression(model, loader, device):
    """攻撃性ヘッドの精度指標: pressure_mae, sign_acc, all_mae。"""
    model.eval()
    all_pred = []; all_label = []
    for batch in loader:
        features         = batch[0].to(device)
        disc_tok         = batch[1].to(device)
        disc_mask        = batch[2].to(device)
        aggression_label = batch[8].to(device)   # batch[8] = aggression_label (batch[7]=noise_mask)
        aggression_logit = model(features, disc_tok, disc_mask)[5]  # [5]=aggression_logit
        all_pred.append(aggression_logit.cpu())
        all_label.append(aggression_label.cpu())
    pred  = torch.cat(all_pred)
    label = torch.cat(all_label)
    pressure_mask = (label != 0.0)
    all_mae       = (pred - label).abs().mean().item()
    pressure_mae  = (pred[pressure_mask] - label[pressure_mask]).abs().mean().item() if pressure_mask.any() else 0.0
    sign_acc      = ((pred[pressure_mask] * label[pressure_mask]) > 0).float().mean().item() if pressure_mask.any() else 0.0
    return {
        "agg_all_mae":      round(all_mae, 4),
        "agg_pressure_mae": round(pressure_mae, 4),
        "agg_sign_acc":     round(sign_acc, 4),
    }


@torch.no_grad()
def save_aggression_context(model, loader, device, save_path):
    """学習後検証用: サンプルごとの局面文脈+攻撃性予測をnpzに保存。

    保存フィールド:
      rank_gaps     (N, 4): 順位差 features[:,0,43:47]
      zhuangfeng    (N,):   場風/3  features[:,0,47]
      jushu_norm    (N,):   局数/4  features[:,0,48]
      remaining     (N,):   残牌/70 features[:,0,50]
      pressure_mask (N,):   True=圧力局面 (label != 0.0)
      pred          (N,):   予測攻撃性 (-1〜+1)
      label         (N,):   真の攻撃性 (-1〜+1)
    """
    model.eval()
    rank_gaps_list = []; zhuang_list = []; jushu_list = []
    remain_list = []; pressure_list = []; pred_list = []; label_list = []
    for batch in loader:
        features         = batch[0].to(device)
        disc_tok         = batch[1].to(device)
        disc_mask        = batch[2].to(device)
        aggression_label = batch[8]   # CPU: aggression_label
        aggression_logit = model(features, disc_tok, disc_mask)[5].cpu()
        f0 = features[:, 0, :].cpu()
        rank_gaps_list.append(f0[:, HI_SCORE_START + 4 : HI_SCORE_START + 8])  # [43:47]
        zhuang_list.append(f0[:, HI_SCORE_START + 8])                           # [47]
        jushu_list.append(f0[:, HI_SCORE_START + 9])                            # [48]
        remain_list.append(f0[:, HI_GAME_START])                                # [50]
        pressure_list.append((aggression_label != 0.0))
        pred_list.append(aggression_logit)
        label_list.append(aggression_label)
    np.savez(
        str(save_path),
        rank_gaps     = torch.cat(rank_gaps_list).numpy(),
        zhuangfeng    = torch.cat(zhuang_list).numpy(),
        jushu_norm    = torch.cat(jushu_list).numpy(),
        remaining     = torch.cat(remain_list).numpy(),
        pressure_mask = torch.cat(pressure_list).numpy(),
        pred          = torch.cat(pred_list).numpy(),
        label         = torch.cat(label_list).numpy(),
    )
    print(f"eval_aggression_context.npz saved: {save_path}", flush=True)
...
def main():
    ...
    if not ndjson_path.exists():
        ...
        print("先に以下を実行してください (v46パイプライン):")
        print("  # v45パイプラインと同じ特徴量抽出・ブロックラベル付与まで実行後")
        print("  python phase2/scripts/add_aggression_labels.py --src <v45_block>.ndjson --states states_v22.ndjson --dst hand_inference_v46.ndjson")
        sys.exit(1)

    print(f"読み込み中: {ndjson_path}", flush=True)

    INPUT_DIM  = CONFIG["fixed_dim"]   # 674
    TOKEN_DIM  = CONFIG["token_dim"]   # 45 (danger_level追加)
    ...
    laggression_np = np.zeros((n_lines,),              dtype=np.float32)
    ...
    disc_tokens_list = []  # List[List[np.ndarray(n,45) float16]]

    noise_cnt=agg_nonzero_cnt=0
    ...
    gc.collect()
    print(f"noise={noise_cnt}  agg_nonzero={agg_nonzero_cnt}")
    ...
    print(f"pw: block={pw_block:.2f} wait={pw_wait:.2f}")
    ...
    full_ds  = HandInferenceDataset(
        ...
        lnoise_np, laggression_np,
        lyaku_np, lwon_np,
    )
    del feat_np, lab_np, lred_np, lblk_np, lsh_np, lnoise_np, laggression_np, lyaku_np, lwon_np
    gc.collect()
    ...
    for epoch in range(start_epoch, CONFIG["epochs"]+1):
        ...
        agg_m                          = eval_aggression(model, val_loader, device)
    ...
    test_agg_m                         = eval_aggression(model, test_loader, device)
    ...
    for k, v in test_agg_m.items():  print(f"  {k}: {v}")

    (MODEL_DIR/"eval_result.json").write_text(json.dumps(
        {"test_eae":test_eae, "test_eae_stage":test_eae_stage,
         "test_acc":test_acc, **test_wait_m, **test_agg_m, "config":CONFIG}, indent=2))
    (MODEL_DIR/"config.json").write_text(json.dumps(
        {**CONFIG, "n_tatsu":N_TATSU, "n_shanten_cls":N_SHANTEN_CLASSES}, indent=2))
    save_aggression_context(model, test_loader, device, MODEL_DIR / "eval_aggression_context.npz")
