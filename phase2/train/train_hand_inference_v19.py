"""
手牌類推モデル v19: Multi-task 手牌構成予測

v15からの変更点:
  - データ: hand_inference_v19.ndjson (label_block 追加: (3, 89))
  - モデル: 構成ヘッド追加 (block_selection_matrix + block_head)
    block_feats = block_sel @ out  → block_head → (B, 3, 89)
  - 損失: + LAMBDA_BLOCK * BCE(block_logits, block_labels)
  - 評価: + block_f1, block_precision, block_recall
  - early stopping: val_eae (v15 と同じ)
  - DP decode: pred_total_mae=0 を保証
"""
...
MODEL_DIR = Path(__file__).parent.parent / "models" / "hand_inference" / "v19"
...
class HandInferenceV19(nn.Module):
    ...
    def forward(self, x):
        ...
        # 構成ヘッド: block_sel (89, 34) @ out (B*P, 34, d) → (B, 3, 89, d)
...
def eval_metrics_detailed(model, loader, device):
    ...
    for features, labels, labels_red, labels_block in loader:
        ...
        blk_pred = (torch.sigmoid(block_logits) > 0.5).cpu()  # (B, 3, 89)
    ...
    # ブロック予測指標
    blk_pred_all = torch.cat(all_blk_pred, dim=0)  # (N*3, 89)
...
def main():
    ...
    # 事前アロケート（Pythonリストを経由しないでメモリを節約）
