"""
手牌類推モデル v43: Uncertainty Weighting + 非公開牌数逆数重み + 複合指標

v42 からの変更:
  1. get_stage_weights → get_inference_weights
     非公開牌数（wall + 相手手牌）の逆数で重みづけ。副露タイルは公開情報扱い。
  2. LAMBDA_XXX を全廃止 → UncertaintyWeights (Kendall et al. 2018) で自動調整
  3. メイン指標を複合化: composite = shanten_acc×0.4 + wait_f1×0.4 + 1/(1+eae/15)×0.2
  4. shanten_acc を eval・ログに追加
  5. v42の副露数ベース yaku 重み付けは継承
"""
...
MODEL_DIR = Path(__file__).parent.parent / "models" / "hand_inference" / "v43"
...
LAMBDA_SUM   = 0.05   # 手牌総枚数 MSE (固定): UW から除外し安定化
LAMBDA_CROSS = 0.1    # 牌種上限制約 (固定): UW から除外し安定化
...
class UncertaintyWeights(nn.Module):
    ...
    L = Σ [ 0.5 * exp(-log_s_i) * L_i + 0.5 * log_s_i ]
    """
    TASK_NAMES = ["nll", "red_ce", "red_cons",
                  "block", "wait", "shanten", "retreat", "push", "furiten", "yaku"]

    def __init__(self):
        super().__init__()
        self.log_s = nn.Parameter(torch.zeros(len(self.TASK_NAMES)))

    def forward(self, losses):
        precision = torch.exp(-self.log_s)
        return (0.5 * precision * torch.stack(losses) + 0.5 * self.log_s).sum()

    def log_weights(self):
        return {name: round(self.log_s[i].item(), 4)
...
def train_epoch(model, loader, optimizer, uw, device,
    ...
    for batch in loader:
        ...
        loss = (uw([loss_nll, loss_red_ce, loss_red_cons,
                    loss_block, loss_wait, loss_shanten, loss_retreat, loss_push,
                    furiten_pen, loss_yaku])
                + LAMBDA_SUM   * loss_sum
                + LAMBDA_CROSS * loss_cross)
        loss.backward()
        torch.nn.utils.clip_grad_norm_(
            list(model.parameters()) + list(uw.parameters()), GRAD_CLIP_NORM)
        optimizer.step()
...
def main():
    ...
    optimizer = torch.optim.AdamW([
        ...
        {"params": uw.parameters(),    "lr": CONFIG["lr"] * 0.1, "weight_decay": 0.0},
    ])
    scheduler    = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode="max", patience=4)
    best_composite = -math.inf; patience_cnt = 0; start_epoch = 1
    ...
    if resume and ckpt_path.exists() and (MODEL_DIR/"train_log.json").exists():
        ...
        best_composite = max((e.get("composite", -math.inf) for e in logs), default=-math.inf)
        start_epoch    = max(e["epoch"] for e in logs) + 1
        patience_cnt   = 0
        print(f"resume: epoch {start_epoch}  best_composite={best_composite:.4f}", flush=True)
    else:
        print("フルスクラッチ学習開始", flush=True)

    for epoch in range(start_epoch, CONFIG["epochs"]+1):
        train_nll        = train_epoch(model, train_loader, optimizer, uw, device,
                                       pos_weight_block, pos_weight_wait, pos_weight_retreat, pos_weight_push)
        ...
        log_entry = {"epoch": epoch, "val_eae": round(val_eae, 4), "val_acc": round(val_acc, 4),
                     "shanten_acc": round(shanten_acc, 4), "composite": round(composite, 4),
                     "log_sigma": uw.log_weights(), **wait_m}
        print(f"epoch {epoch:3d}  nll={train_nll:.4f}  eae={val_eae:.4f}  shanten={shanten_acc:.4f}"
        ...
        if composite > best_composite:
            best_composite = composite; patience_cnt = 0
            torch.save(model.state_dict(), MODEL_DIR/"model.pt")
            print(f"  [saved best_composite={best_composite:.4f}]", flush=True)
        else:
            patience_cnt += 1
