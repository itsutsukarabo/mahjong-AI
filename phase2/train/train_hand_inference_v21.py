"""
手牌類推モデル v21: block 損失の過剰を修正

v20からの変更点:
  - LAMBDA_BLOCK: 1.0 → 0.3 (block を補助タスクに戻す)
  - pos_weight: 自動計算値を上限 10 でクリップ (v20: 32.73 が強すぎた)

v20 の反省:
  pos_weight=32.73 × LAMBDA_BLOCK=1.0 ≈ 33 でブロック損失が NLL を圧迫し、
  block_recall=0.919 だが block_precision=0.183 に崩壊。
  手牌推測 test_eae も 6.70 → 7.71 に悪化。
"""
...
from enumerate_decompositions import build_block_selection_matrix, N_BLOCKS

# ---- 設定 ----

DATA_DIR  = Path(__file__).parent.parent / "data" / "features"
MODEL_DIR = Path(__file__).parent.parent / "models" / "hand_inference" / "v21"
...
LAMBDA_RED_CE      = 0.3
LAMBDA_RED_CONS    = 0.1
LAMBDA_BLOCK       = 0.3   # v20: 1.0 → v21: 0.3（block を補助タスクに戻す）
POS_WEIGHT_MAX     = 10.0  # v21: pos_weight の上限クリップ（v20: 32.73 が強すぎた）
GRAD_CLIP_NORM     = 1.0
...
def dp_decode(logits, k):
    """DP で Σcount_i = k を厳密に満たすよう argmax を求める (pred_total_mae=0 保証)"""
    B, T, C = logits.shape
    NEG_INF = float('-inf')
...
class HandInferenceDataset(Dataset):
    def __init__(self, features, labels, labels_red, labels_block):
    ...
    def __getitem__(self, idx):
        return self.features[idx], self.labels[idx], self.labels_red[idx], self.labels_block[idx]


# ---- モデル ----

class HandInferenceV21(nn.Module):
    def __init__(self, input_dim, d_model, nhead, num_layers,
                 n_pai=34, n_count_cls=5, n_players=3, n_blocks=89, dropout=0.1):
        super().__init__()
        self.n_pai       = n_pai
        ...
        # block_head 2層化 (v19: Linear(d,1) → v20: Linear(d,64)→ReLU→Linear(64,1))
        ...
        block_sel = torch.tensor(build_block_selection_matrix(), dtype=torch.float32)
        self.register_buffer("block_sel", block_sel)  # (89, 34)
    ...
    def forward(self, x):
        ...
        out_flat    = out.reshape(B * P, self.n_pai, -1)           # (B*P, 34, d)
        block_feats = torch.bmm(
            self.block_sel.unsqueeze(0).expand(B * P, -1, -1),    # (B*P, 89, 34)
            out_flat,                                              # (B*P, 34, d)
        ).reshape(B, P, self.n_blocks, -1)                        # (B, 3, 89, d)
...
def train_epoch(model, loader, optimizer, device, pos_weight):
    ...
    for features, labels, labels_red, labels_block in loader:
        ...
        # pos_weight 付き BCE (v20: class imbalance 対策)
...
def main():
    ...
    ndjson_path = DATA_DIR / "hand_inference_v19.ndjson"
    if not ndjson_path.exists():
        print(f"ファイルなし: {ndjson_path}")
        print("python phase2/scripts/add_block_labels.py を先に実行してください")
        sys.exit(1)
    ...
    print(f"features: {feat_np.shape}, label_block: {lblk_np.shape}")

    # pos_weight: 訓練データの正例率から自動計算
    ...
    train_dataset = HandInferenceDataset(feat_np[sl_tr], lab_np[sl_tr], lred_np[sl_tr], lblk_np[sl_tr])
    val_dataset   = HandInferenceDataset(feat_np[sl_va], lab_np[sl_va], lred_np[sl_va], lblk_np[sl_va])
    test_dataset  = HandInferenceDataset(feat_np[sl_te], lab_np[sl_te], lred_np[sl_te], lblk_np[sl_te])
    del feat_np, lab_np, lred_np, lblk_np; gc.collect()
