"""
手牌類推モデル v38: 統一イベントストリーム (DISCARD + PASS_PON + PASS_CHI) Transformer
...
v37 からの変更:
  ...
  4. データファイル: hand_inference_v38.ndjson
"""
...
# v38 レイアウトに対応したオフセット（特徴量レイアウトはv37と同一）
...
class HandInferenceV37(nn.Module):
    """
    v38: DiscardEncoder + タイルレベル Transformer (統一イベントストリーム)
...
def main():
    ...
    ndjson_path = DATA_DIR / "hand_inference_v38.ndjson"
    if not ndjson_path.exists():
        ...
        print("  python phase2/scripts/prepare_v38_data.py")
        print("  python phase2/scripts/add_block_labels.py --src hand_inference_v38_raw.ndjson --dst hand_inference_v38.ndjson --v2 --include-tatsu")
