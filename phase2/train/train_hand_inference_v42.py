"""
手牌類推モデル v42: yaku損失を副露数に応じた重み付きに変更

v41 からの変更:
  1. LAMBDA_YAKU: 0.3 → 0.1 (全体的な役損失の比率を下げる)
  2. yaku損失を副露数に応じた per-sample 重み付きBCEに変更
     副露0 → ×1.0、副露1 → ×2.0、副露2 → ×3.0、副露3 → ×4.0、副露4 → ×0.0
     (副露が多いほど手牌情報が外部に露出 → 役推定の信頼性が高いため重視)
     (副露4は特殊ケースのため除外)
  3. データファイル: hand_inference_v41.ndjson (v41と同じデータを使用)
"""
...
MODEL_DIR = Path(__file__).parent.parent / "models" / "hand_inference" / "v42"
...
def train_epoch(model, loader, optimizer, device,
    ...
    for batch in loader:
        ...
        if won_mask.any():
            ...
            yaku_w = _YAKU_MELD_W[meld_cnt]          # (B,)
            won_w  = yaku_w[won_mask]                 # (N_won,)
            loss_per = F.binary_cross_entropy_with_logits(
                yaku_logits[:, 0, :][won_mask], labels_yaku[won_mask],
                reduction="none"
            ).mean(dim=-1)                            # (N_won,)
            denom = won_w.sum()
            loss_yaku = (loss_per * won_w).sum() / denom if denom > 0 else torch.tensor(0.0, device=device)
        else:
