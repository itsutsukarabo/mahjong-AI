"""
手牌類推モデル v45: v44 土台バグ修正 + danger_level 特徴量追加

v44 からの変更:
  (a) 入力バグ修正3点 (extract_features.js)
      - seat_off = l (旧: dealer=jushu%4 により 75%の局で巡目順序が誤り)
      - jikaze = player_l (旧: (player_l - jushu + 4) % 4 により風17次元が誤り)
  (b) push/retreatラベル修正 (add_intent_labels.py)
      - get_riichi_suji: '_' in ts → '*' in ts
        (旧版は手出しリーチ94.9%でスジ誤計算)
  (c) danger_level 特徴量追加: token_dim 44 → 45
      - 0=リーチなし, 1=現物, 2=全員スジ, 3=無スジ
  データ: hand_inference_v41.ndjson → hand_inference_v45.ndjson
  モデルアーキ・λ・batch/lr/epoch: v44と完全同一
  ベースライン: v44 eae=3.89 (差分は3変更の複合)
"""
...
MODEL_DIR = Path(__file__).parent.parent / "models" / "hand_inference" / "v45"
...
def main():
    ...
    ndjson_path = DATA_DIR / "hand_inference_v45.ndjson"
    if not ndjson_path.exists():
        print(f"ファイルなし: {ndjson_path}")
        print("先に以下を実行してください (v45パイプライン):")
        ...
        print("  (重複除去: dedup_hand_inference.py → hand_inference_dedup.ndjson)")
        print("  python phase2/scripts/prepare_v41_data.py --src hand_inference_dedup.ndjson --dst hand_inference_v45_raw.ndjson")
        print("  python phase2/scripts/add_block_labels.py --src hand_inference_v45_raw.ndjson --dst hand_inference_v45_block.ndjson --v2 --include-tatsu")
        print("  python phase2/scripts/add_intent_labels.py --src hand_inference_v45_block.ndjson --states states_v22.ndjson --dst hand_inference_v45.ndjson")
        sys.exit(1)
