"""
手牌類推モデル v27: v23失敗修正版 (N_BLOCKS=89, ターツなし, 全シャンテン数対応, 排他的バックトラッキング)
...
v23からの変更点:
  ...
  - データ: hand_inference_v27.ndjson (label_shanten フィールドを含む)
"""
...
from enumerate_decompositions import (
    build_block_selection_matrix, N_BLOCKS
)

# ---- 設定 ----

DATA_DIR  = Path(__file__).parent.parent / "data" / "features"
MODEL_DIR = Path(__file__).parent.parent / "models" / "hand_inference" / "v27"
...
class HandInferenceV26(nn.Module):
    def __init__(self, input_dim, d_model, nhead, num_layers,
        ...
        block_sel = torch.tensor(
            build_block_selection_matrix(include_tatsu=False), dtype=torch.float32
        )
...
def main():
    ...
    ndjson_path = DATA_DIR / "hand_inference_v27.ndjson"
    if not ndjson_path.exists():
        print(f"ファイルなし: {ndjson_path}")
        print("python phase2/scripts/add_block_labels.py --v2 を先に実行してください")
        sys.exit(1)
