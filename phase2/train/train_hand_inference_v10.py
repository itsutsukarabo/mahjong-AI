"""
手牌類推モデル v10: v9 から input_dim を 371→374 に拡大（TICKET-030: red_visible 追加）

入力 (374次元):
  target_discard(44)  [index   0: 44]
  target_meld(38)     [index  44: 82]
  riichi(1)           [index  82: 83]
  score(11)           [index  83: 94]
  game(9)             [index  94:103]  ← features[94] = remaining/70
  self_discard(44)    [index 103:147]
  self_meld(38)       [index 147:185]
  visible_counts(34)  [index 185:219]
  red_discard_sig(3)  [index 219:222]
  red_visible(3)      [index 222:225]  ← NEW (TICKET-030)
  other1_discard(44)  [index 225:269]
  other2_discard(44)  [index 269:313]
  pass_pon_signal(34) [index 313:347]
  wind(5)             [index 347:352]
  yaku_prob(21)       [index 352:373]
  tenpai_prob(1)      [index 373:374]

アーキテクチャ (v9 と同一):
  1. global_encoder (input_dim->256->d_model + BN): 全特徴量のグローバル圧縮
...
評価 (TICKET-034): remaining バケット別 accuracy を eval_result.json に追記
  early   : remaining > 50  (1～7巡目頃)
  mid     : 30 < remaining ≤ 50  (8～14巡目頃)
  late    : 10 < remaining ≤ 30  (15～21巡目頃)
  endgame : remaining ≤ 10  (22巡目以降)
...
MODEL_DIR = Path(__file__).parent.parent / "models" / "hand_inference" / "v10"
...
VISIBLE_OFFSET     = 185   # visible_counts は index 185-218
REMAINING_OFFSET   = 94    # features[94] = remaining / 70
GPU_TEMP_THRESHOLD = 65    # °C を超えたら冷却待機
...
# TICKET-034: remaining バケット定義
REMAINING_BUCKETS = {
    "early":   (51, 9999),   # remaining > 50
    "mid":     (31,   50),   # 30 < remaining <= 50
    "late":    (11,   30),   # 10 < remaining <= 30
    "endgame": ( 0,   10),   # remaining <= 10
}
...
def eval_by_remaining(model, loader, device):
    """TICKET-034: remaining バケット別 accuracy を計算する。"""
    model.eval()
    ...
    for features, labels, labels_red in loader:
        ...
        correct_mask = (preds == labels)                                       # [B, 34]
        loss_per_sample = F.cross_entropy(
            logits_raw.reshape(-1, model.n_count_cls),
            labels.reshape(-1),
            reduction="none",
        ).reshape(len(features), -1).mean(dim=-1)                             # [B]
    ...
    for name, b in buckets.items():
        n = b["total"] // model.n_pai if model.n_pai > 0 else 1  # サンプル数
        result[name] = {
