"""
手牌類推モデル v20: Multi-task 手牌構成予測 (改善版)

v19からの変更点:
  - ラベル: 七対子テンパイバグ修正 + 浮き対子ラベル化 (enumerate_decompositions.py 修正済み)
  - 損失: pos_weight 付き BCE (block_recall=0.184 の改善)
  - 損失: 訓練時も終盤重み付き NLL (従来は eval のみ)
  - モデル: block_head 2層化 (Linear→ReLU→Linear)
...
MODEL_DIR = Path(__file__).parent.parent / "models" / "hand_inference" / "v20"
...
LAMBDA_RED_CE      = 0.3
LAMBDA_RED_CONS    = 0.1
LAMBDA_BLOCK       = 1.0   # v19: 0.5 → v20: 1.0
GRAD_CLIP_NORM     = 1.0
...
class HandInferenceV20(nn.Module):
...
def main():
    ...
    pw_val = (1.0 - pos_rate) / (pos_rate + 1e-9)
