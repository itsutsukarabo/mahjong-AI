"""
手牌類推モデル v36: v35のバグ修正版（フルスクラッチ）

v35からの修正:
  1. features[441] 復活 (外部tenpai_prob を使用、0固定を解除)
  2. shanten_head: 4クラス → 3クラス (0=テンパイ, 1=1向聴, 2=2向聴以上)
  3. retreat_head / push_head: global_pool(相手3人の平均) → self_encoder
       player l 自身の文脈特徴量 139次元を使用
       features[83:219]  = score(11)+game_state(9)+self_discard(44)+self_meld(38)+visible_counts(34)
       features[222:225] = red_visible(3)
  4. LAMBDA 調整: SHANTEN 0.4→0.2, RETREAT 0.2→0.1, PUSH 0.3→0.1
"""
...
MODEL_DIR = Path(__file__).parent.parent / "models" / "hand_inference" / "v36"
...
GPU_COOL_TARGET    = 70  # エポック後インターバル: この温度まで下がってから次エポック開始
N_PAI              = 34
...
LAMBDA_BLOCK       = 0.3
LAMBDA_WAIT        = 0.5
LAMBDA_SHANTEN     = 0.2   # v35: 0.4 → 0.2
LAMBDA_RETREAT     = 0.1   # v35: 0.2 → 0.1
LAMBDA_PUSH        = 0.1   # v35: 0.3 → 0.1
POS_WEIGHT_MAX     = 10.0
GRAD_CLIP_NORM     = 1.0
...
# player l 自身の文脈特徴量インデックス (全target playerで共通)
# features[83:219]  = score(11)+game_state(9)+self_discard(44)+self_meld(38)+visible_counts(34) = 136
# features[222:225] = red_visible(3)
SELF_FEAT_DIM = 139
...
def cool_after_epoch():
    """エポック終了後に GPU_COOL_TARGET 以下になるまで必ず待機する。"""
    temp=gpu_temp()
...
class HandInferenceV36(nn.Module):
    """
    v35バグ修正版:
      - features[441] 有効（外部tenpai_prob使用）
      - shanten_head: 3クラス (0=テンパイ, 1=1向聴, 2=2向聴以上)
      - retreat/push: self_encoder (player l自身の139次元) を使用
    """
    ...
    def __init__(self, input_dim, d_model, nhead, num_layers,
        ...
        # シャンテン推定ヘッド (per-player): 3クラス
        self.shanten_head = nn.Sequential(
            nn.Linear(d_model, 32), nn.ReLU(), nn.Linear(32, n_shanten_cls)
        )

        # retreat/push ヘッド: player l 自身の文脈から推定
        # x[:, 0, 83:219] = score(11)+game_state(9)+self_discard(44)+self_meld(38)+visible_counts(34) = 136
        # x[:, 0, 222:225] = red_visible(3)  → 合計 139次元
    ...
    def forward(self, x):
        ...
        shanten_logits = self.shanten_head(pooled)                   # (B, P, 3)

        # player l 自身の文脈: 全 target player で共通の部分を使用
        self_feat     = torch.cat([x[:, 0, 83:219], x[:, 0, 222:225]], dim=-1)  # (B, 139)
        self_h        = self.self_encoder(self_feat)                              # (B, 32)
        retreat_logit = self.retreat_head(self_h).squeeze(-1)                    # (B,)
        push_logit    = self.push_head(self_h).squeeze(-1)                       # (B,)
...
def train_epoch(model, loader, optimizer, device,
    ...
    for batch in loader:
        ...
        logits, logits_raw, red_logits, block_logits, wait_logits, \
            retreat_logit, push_logit, shanten_logits = model(features)
        ...
        # shanten (3クラス, 全サンプル, ノイズマスク適用)
        sh_label_3cls = labels_shanten.clamp(0, N_SHANTEN_CLASSES - 1)  # 2以上→class2
        ...
        # retreat / push (player l の self_encoder から)
...
def main():
    ...
    print(f"noise={noise_cnt}({noise_cnt/n_lines*100:.1f}%)  "
          f"retreat={retreat_cnt}({retreat_cnt/n_lines*100:.2f}%)  "
          f"push={push_cnt}({push_cnt/n_lines*100:.2f}%)")
    ...
    lab_sum=lab_np.sum(axis=-1); meld_sum=feat_np[:,:,78]+feat_np[:,:,79]+feat_np[:,:,80]
    ...
    rng=np.random.default_rng(42); idx=rng.permutation(len(feat_np))
    ...
    model = HandInferenceV36(
        ...
        n_shanten_cls=N_SHANTEN_CLASSES, dropout=CONFIG["dropout"],
    ).to(device)
    ...
    if resume and ckpt_path.exists() and (MODEL_DIR/"train_log.json").exists():
        ...
        print(f"resume: epoch {start_epoch}  best_val_eae={best_val_eae:.4f}  patience_cnt=0", flush=True)
    elif resume and (MODEL_DIR/"model.pt").exists() and (MODEL_DIR/"train_log.json").exists():
        # 旧形式 (optimizer 状態なし) からのフォールバック
        ...
        patience_cnt = 0
        start_epoch  = last_epoch + 1
        print(f"resume (legacy, no optimizer state): epoch {start_epoch}  best_val_eae={best_val_eae:.4f}", flush=True)
    else:
        print("フルスクラッチ学習開始（チェックポイントなし）", flush=True)
    ...
    for epoch in range(start_epoch, CONFIG["epochs"]+1):
        ...
        # optimizer/scheduler 状態を毎エポック保存（クラッシュ時の再開用）
        torch.save({
    ...
    # ONNX エクスポート (5出力: +shanten_logits(1,3,3))
    model.eval().cpu()
    ...
    class _Wrap(nn.Module):
        ...
        def forward(self, x):
            logits,_,red_logits,block_logits,wait_logits,_,_,shanten_logits = self.m(x)
    ...
    torch.onnx.export(
        ...
        dynamic_axes={"features":{0:"batch"},"logits":{0:"batch"},"red_logits":{0:"batch"},
                      "block_logits":{0:"batch"},"wait_logits":{0:"batch"},"shanten_logits":{0:"batch"}},
        opset_version=17,
    )
    print(f"\nONNX保存: {MODEL_DIR/'model.onnx'}")
    print(f"出力: logits(1,3,34,5) red_logits(1,3,3,2) block_logits(1,3,134) wait_logits(1,3,113) shanten_logits(1,3,3)")
