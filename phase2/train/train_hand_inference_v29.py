"""
手牌類推モデル v29: v26改良版 (block_loss限定 + stage_w副露補正)

v26からの変更点:
  - block_loss: shanten<=1 のみ適用（shanten>=2 はゼロ = 勾配なし）
      shanten=0: 重み1.0 / shanten=1: 重み0.5 / shanten>=2: 重み0.0
  - get_stage_weights: 副露による公開牌数を傾斜に反映
      副露1枚 = 壁牌2枚相当として有効残り牌数を補正（係数k=2）
  - NLL loss: stage_w のみ（シャンテン重みなし = v26と同一）
  - データ: hand_inference_v26.ndjson を流用（再生成不要）
  - ブロック: N_BLOCKS=134（ターツ含む）を維持
"""
...
def get_stage_weights(features):
    ...
    # 副露による公開牌数（target playerの生カウント）
    # features[:, 78]=n_chi / features[:, 79]=n_pon / features[:, 80]=n_kan
    meld_tiles = (features[:, 78] * 3 + features[:, 79] * 3 + features[:, 80] * 4)

    # 副露1枚 = 壁牌2枚相当として有効残り牌数を補正
