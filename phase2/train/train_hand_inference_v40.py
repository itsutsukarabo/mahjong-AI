"""
手牌類推モデル v40: yaku_head 統合・固定特徴量 695→674次元

v40 からの変更:
  1. yaku_prob 入力 (21次元) を廃止 → fixed_dim 695→674
  2. yaku_head (Linear 256→21) をモデルに統合
     捨て牌トークン列全体を見た上で役を推定 (Transformer backbone 共有)
  3. 損失に LAMBDA_YAKU * BCE(yaku_logits, label_yaku) を追加 (和了サンプルのみ)
  4. データファイル: hand_inference_v40.ndjson (label_yaku / label_won フィールド付き)
"""
...
MODEL_DIR = Path(__file__).parent.parent / "models" / "hand_inference" / "v40"
...
# v40 レイアウトに対応したオフセット（yaku_prob廃止・673+tenpai=674次元）
...
class HandInferenceV37(nn.Module):
    """
    v40: DiscardEncoder + タイルレベル Transformer (統一イベントストリーム) + yaku_head 統合
...
def train_epoch(model, loader, optimizer, device,
    ...
    for batch in loader:
        ...
        # 役損失: 和了サンプル (target player p=0) のみで計算
        won_mask = labels_won  # (B,) bool
        if won_mask.any():
            loss_yaku = F.binary_cross_entropy_with_logits(
...
def main():
    ...
    ndjson_path = DATA_DIR / "hand_inference_v40.ndjson"
    if not ndjson_path.exists():
        ...
        print("  python phase2/scripts/prepare_v40_data.py")
        print("  python phase2/scripts/add_block_labels.py --src hand_inference_v40_raw.ndjson --dst hand_inference_v40.ndjson --v2 --include-tatsu")
