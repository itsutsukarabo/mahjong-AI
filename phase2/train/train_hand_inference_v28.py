"""
手牌類推モデル v28: v26改良版 (block_loss限定 + NLLシャンテン重み)

v26からの変更点:
  - block_loss: 全シャンテン数 → shanten<=1 (テンパイ・1シャンテン) のみ適用
      shanten=0: 重み1.0 / shanten=1: 重み0.5 / shanten>=2: 重み0.0 (勾配なし)
      → 高シャンテン時のフラットなラベルによる雑音を完全排除
  - NLL loss: 残り牌重みに加えてシャンテン重みを乗算
      combined_w = stage_w × (0.5^shanten).clamp(min=0.25)
      → テンパイ・1シャンテン予測に学習を集中
  - データ: hand_inference_v26.ndjson を流用 (再生成不要)
  - ブロック: N_BLOCKS=134 (ターツ含む) を維持 — 将来の当たり牌推測に対応
"""
...
MODEL_DIR = Path(__file__).parent.parent / "models" / "hand_inference" / "v28"
...
def train_epoch(model, loader, optimizer, device, pos_weight):
    ...
    for features, labels, labels_red, labels_block, labels_shanten in loader:
        ...
        # block_loss: shanten<=1 のみ適用（shanten>=2 はゼロ = 勾配なし）
        # shanten=0(テンパイ): 重み1.0 / shanten=1: 重み0.5 / shanten>=2: 重み0.0
        block_w = torch.where(
            labels_shanten <= 1,
...
def eval_epoch(model, loader, device):
    ...
    for features, labels, labels_red, labels_block, labels_shanten in loader:
        ...
        for p in range(3):
            ...
            total_wsum += eae_val.item() * combined_w.sum().item()
            total_w    += combined_w.sum().item()
...
def main():
    ...
    ndjson_path = DATA_DIR / "hand_inference_v26.ndjson"  # v26データを流用
