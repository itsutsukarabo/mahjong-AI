"""
手牌類推モデル v37: 捨て牌 Transformer (DiscardEncoder) + タイルレベル Transformer

v36 からの変更:
  1. 入力次元 442 → 695 (固定特徴量)
  2. 捨て牌トークン列 (42次元/トークン, 可変長) を DiscardEncoder で符号化
  3. DiscardEncoder 出力 (64次元) を固定特徴量に結合 → global_encoder
  4. VISIBLE_OFFSET / REMAINING_OFFSET / SELF_FEAT_DIM を v37 レイアウトに更新
  5. DataLoader に variable-length token 対応の collate_fn を追加
"""
...
MODEL_DIR = Path(__file__).parent.parent / "models" / "hand_inference" / "v37"

CONFIG = {
    "fixed_dim":    HI_TOTAL,   # 695
    "token_dim":    HI_TOKEN_DIM,  # 42
    "disc_d":       64,    # DiscardEncoder 出力次元
    "disc_nhead":   4,
    "disc_layers":  2,
    "d_model":      256,
    "nhead":        4,
...
class DiscardEncoder(nn.Module):
    """捨て牌トークン列 → コンテキストベクトル (disc_d次元)"""

    def __init__(self, token_dim=42, disc_d=64, nhead=4, num_layers=2, dropout=0.1):
        super().__init__()
...
class HandInferenceV37(nn.Module):
    """
    v37: DiscardEncoder + タイルレベル Transformer
...
def main():
    ...
    ndjson_path = DATA_DIR / "hand_inference_v37.ndjson"
    if not ndjson_path.exists():
        ...
        print("  python phase2/scripts/prepare_v37_data.py")
        print("  python phase2/scripts/add_block_labels.py --src hand_inference_v37_raw.ndjson --dst hand_inference_v37.ndjson --v2 --include-tatsu")
    ...
    print("入力: features(1,3,695), disc_tokens(1,3,N,42), disc_mask(1,3,N)")
