"""
手牌類推モデル v14: v10アーキテクチャ + val_eae early stopping

v10と同一:
  - アーキテクチャ (block heads なし)
  - 学習損失: NLL + MSE合計ペナルティ (lambda_global_sum=0.05)

v10からの変更点:
  - early stopping指標: val_loss → val_eae (終盤重み付きEAE)
  - データ読み込み: numpy streaming (OOM対策)
  - ノイズ除外: 手牌合計0枚 かつ 副露なしのサンプルを除外
  - gpu_temp(): WSL/Windows両対応
  - 勾配クリッピング追加 (norm=1.0)
  - 詳細評価指標追加 (eval_metrics_detailed)
"""
...
MODEL_DIR = Path(__file__).parent.parent / "models" / "hand_inference" / "v14"
...
class HandInferenceV10(nn.Module):
...
def train_epoch(model, loader, optimizer, device):
    ...
    for features, labels, labels_red in loader:
        ...
        loss_global    = F.mse_loss(expected_total, labels.float().sum(-1))
...
def eval_epoch(model, loader, device):
    """Returns (val_eae, val_acc): val_eae が early stopping 指標"""
    model.eval()
    total_wsum = 0.0
    total_w    = 0.0
    correct    = 0
