"""
手牌類推モデル v8: v7 から yaku_prob(21) + tenpai_prob(1) の末尾22次元を除外 (ablation B)
...
入力 (349次元):
  ...
  ※ データは371次元のまま。末尾22次元(yaku_prob+tenpai_prob)をスライスで無視する。
...
MODEL_DIR = Path(__file__).parent.parent / "models" / "hand_inference" / "v8"
...
class HandInferenceDataset(Dataset):
    def __init__(self, data):
        self.features   = torch.tensor(
            [s["features"][:CONFIG["input_dim"]] for s in data], dtype=torch.float32
        )
...
def main():
    ...
    if sample_dim < CONFIG["input_dim"]:
        print(f"次元数不足: expected >= {CONFIG['input_dim']}, got {sample_dim}")
        sys.exit(1)
    print(f"特徴量次元: {sample_dim} → 先頭 {CONFIG['input_dim']} 次元を使用（末尾{sample_dim - CONFIG['input_dim']}次元を除外）")
