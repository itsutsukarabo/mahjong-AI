"""
手牌類推モデル v35: フルスクラッチ、全機能統合

v32(フルスクラッチ)との比較で intent labelsの純粋な効果を測定する。

v32 からの追加:
  - features[441] = 0固定 (外部tenpai_prob廃止、自己推定に切り替え)
  - label_noise (①): loss_weight=0
  - retreat_head (④): binary BCE, LAMBDA=0.2
  - push_head    (③): binary BCE, LAMBDA=0.3
  - shanten_head    : 4クラスCE (0=テンパイ,1=1向聴,2=2向聴,3=3+向聴), LAMBDA=0.4
  - ONNX出力: 5出力 (+shanten_logits shape:(1,3,4))
  - early_stop_patience: 15 (フルスクラッチ向け)

ブラウザへの反映 (v35完了後):
  - shanten_logits の softmax[class=0] = テンパイ確率
  - 外部 tenpai_inference モデル不要になる
"""
...
MODEL_DIR = Path(__file__).parent.parent / "models" / "hand_inference" / "v35"

CONFIG = {
    "input_dim":    442,      # features[441]=0固定だが次元は保持（weight収束で無効化）
    "d_model":      256,
    "nhead":        4,
    "num_layers":   3,
    "n_pai":        34,
    ...
    "n_blocks":     N_BLOCKS,
    "dropout":      0.1,
    "lr":           5e-4,     # フルスクラッチなので標準LR
    "weight_decay": 1e-4,
    "batch_size":   256,
    "epochs":       200,
    "early_stop_patience": 15,  # フルスクラッチ向けに長め
}
...
TENPAI_FEAT_IDX    = 441    # features[441]を0に固定するインデックス
...
class HandInferenceV35(nn.Module):
    """フルスクラッチ、全補助タスク統合: retreat(④) + push(③) + shanten"""
    ...
    def __init__(self, input_dim, d_model, nhead, num_layers,
        ...
        # 補助ヘッド (global_pool 共有)
        self.retreat_head = nn.Linear(d_model, 1)
        self.push_head    = nn.Linear(d_model, 1)

        # シャンテン推定ヘッド (per-player, pooled から)
        # クラス: 0=テンパイ, 1=1向聴, 2=2向聴, 3=3+向聴
        self.shanten_head = nn.Sequential(
            nn.Linear(d_model, 32), nn.ReLU(), nn.Linear(32, 4)
        )
    ...
    def forward(self, x):
        ...
        shanten_logits = self.shanten_head(pooled)                     # (B, P, 4)
...
def train_epoch(model, loader, optimizer, device,
    ...
    for batch in loader:
        ...
        # shanten (全サンプル、ノイズマスク適用)
        loss_shanten = (F.cross_entropy(
            shanten_logits.reshape(-1,4), labels_shanten.clamp(0,3).reshape(-1), reduction='none'
...
def main():
    ...
    print(f"noise={noise_cnt}({noise_cnt/n_lines*100:.1f}%)  retreat={retreat_cnt}({retreat_cnt/n_lines*100:.2f}%)  push={push_cnt}({push_cnt/n_lines*100:.2f}%)")

    # features[441] (外部tenpai_prob) を無効化
    feat_np[:, :, TENPAI_FEAT_IDX] = 0.0
    print("features[441] (tenpai_prob) を 0 に固定")
    ...
    # ONNX エクスポート (5出力: +shanten_logits)
    model.eval().cpu()
    ...
    print(f"出力: logits(1,3,34,5) red_logits(1,3,3,2) block_logits(1,3,134) wait_logits(1,3,113) shanten_logits(1,3,4)")
