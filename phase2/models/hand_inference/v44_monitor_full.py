"""v44 full monitor: ep31 -> early_stop
- 安全弁: train_nll 2epoch連続上昇 → 停止報告
- ep100: 報告のみ（停止しない）
- プロセス終了（early stopping）: 最終報告
"""
import re, time, os, subprocess, json

LOG  = r"C:\Users\Administrator.THPC-L01\mahjong-AI\phase2\models\hand_inference\v44_resume2_stdout.log"
...
PIDF = r"C:\Users\Administrator.THPC-L01\mahjong-AI\phase2\models\hand_inference\v44_training2.pid"
OUT  = r"C:\Users\Administrator.THPC-L01\mahjong-AI\phase2\models\hand_inference\v44_monitor2_output.log"
...
def final_report(nll_history):
    ...
    if nll_history:
        ...
        p(f"  v39 ep200=0.0591  v39 final best≈0.059  v43 ep68=0.5056")
    p("[monitor] === END ===")
...
def main():
    p("[monitor] Full monitor started (ep31 -> early_stop)")
    ...
    while True:
        ...
        for ep, nll in new_eps:
            ...
            if ep >= 31 and len(nll_history) >= 3:
                _, n2 = nll_history[-1]
                _, n1 = nll_history[-2]
                _, n0 = nll_history[-3]
                if n2 > n1 and n1 > n0:
