...
hand_inference_v41_raw.ndjson (3プレイヤー結合、discard_tokens付き) に変換する。
...
使い方:
  python prepare_v41_data.py
  python prepare_v41_data.py --src foo.ndjson --dst bar.ndjson
...
OUTPUT_PATH  = Path(_opts["dst"]) if "dst" in _opts else FEATURES_DIR / "hand_inference_v41_raw.ndjson"
